...
public class TestS3AsyncOutputStream {
  private static final Logger LOG = LoggerFactory.getLogger(TestS3AsyncOutputStream.class);
  ...
  public TestS3AsyncOutputStream() throws IOException {}
  ...
  public void testAbortAfterFailedPartUpload() {
    ...
    future.completeExceptionally(mockException);
    doReturn(future).when(s3mock).uploadPart((UploadPartRequest) any(), (AsyncRequestBody) any());

    Assertions.assertThatThrownBy(
            () -> {
              try (S3AsyncOutputStream stream =
                  new S3AsyncOutputStream(s3mock, randomURI(), properties, nullMetrics())) {
  ...
  public void testMultipleClose() throws IOException {
    S3AsyncOutputStream stream =
        new S3AsyncOutputStream(s3, randomURI(), properties, nullMetrics());
    stream.close();
    stream.close();
  }
        ...
        .forEach(
            arrayWrite -> {
              ...
              verify(s3mock, times(1))
                  .putObject(putObjectRequestArgumentCaptor.capture(), (AsyncRequestBody) any());
              ...
              verify(s3mock, times(2))
                  .uploadPart(uploadPartRequestArgumentCaptor.capture(), (AsyncRequestBody) any());
  ...
  private void writeAndVerify(S3AsyncClient client, S3URI uri, byte[] data, boolean arrayWrite) {
    try (S3AsyncOutputStream stream =
        new S3AsyncOutputStream(client, uri, properties, nullMetrics())) {
      if (arrayWrite) {
        stream.write(data);
  ...
  private byte[] readS3Data(S3URI uri) {
    CompletableFuture<ResponseBytes<GetObjectResponse>> data =
        s3.getObject(
            GetObjectRequest.builder().bucket(uri.bucket()).key(uri.key()).build(),
            AsyncResponseTransformer.toBytes());

    return data.join().asByteArray();
  }
